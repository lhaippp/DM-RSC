...
def get_occu_mask_backward(flow, th=0.5):
    b, c, h, w = flow.size()
...
def forward_warp(img, flow):
    """
    一个简单的forward warp实现（速度较慢，仅适合演示）
    """
    b, c, h, w = img.size()
    device = img.device
    ...
    # 计算目标坐标位置
    x_target = x_grid + flow[:, 0]
    y_target = y_grid + flow[:, 1]

    # 将坐标取整
    x_target_round = torch.round(x_target).long()
    y_target_round = torch.round(y_target).long()

    # 创建目标图像
    warped_img = torch.zeros_like(img)

    # 有效坐标掩码
    mask = (x_target_round >= 0) & (x_target_round < w) & (y_target_round >= 0) & (y_target_round < h)

    for b_idx in range(b):
        # 有效的像素索引
        valid_x = x_target_round[b_idx][mask[b_idx]]
        valid_y = y_target_round[b_idx][mask[b_idx]]

        # 源图像的有效像素
        source_pixels = img[b_idx, :, mask[b_idx]]

        # 放置像素（遇到重合像素会直接覆盖；实际应用通常用soft splatting）
        warped_img[b_idx, :, valid_y, valid_x] = source_pixels

    return warped_img

def backward_warp(img, flow):
    """
    Backward warp: 根据flow从目标位置向源图像(img)采样，插值得到扭曲后的图像。
    Args:
        img: Tensor, [b, 3, h, w]
        flow: Tensor, [b, 2, h, w]，表示(x, y)方向的位移
    Returns:
        warped_img: Tensor, [b, 3, h, w]
    """
    b, c, h, w = img.size()
    device = img.device
    ...
    grid_x = x.unsqueeze(0).expand(b, -1, -1) + flow[:, 0]
    grid_y = y.unsqueeze(0).expand(b, -1, -1) + flow[:, 1]
