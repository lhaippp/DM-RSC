...
class ODM(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample(self, classes, condition,shape, mask, rs, cond_scale = 1., rescaled_phi = 0, clip_denoised = True):#
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            x_start = Ap(rs) + mask * x_start  # A†y + (I - A†A)x0|t
            ...
            img1 = unnormalize_to_zero_to_one(img)
    ...
    @torch.no_grad()
    def sample(self, classes, condition, mask, rs, cond_scale=6., rescaled_phi=0.7):  
        
        condition = normalize_to_neg_one_to_one(condition)
        rs = normalize_to_neg_one_to_one(rs)
        batch_size, h,w, channels = classes.shape[0], condition.shape[2], condition.shape[3],self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(classes, condition,(batch_size, (channels), h, w), mask, rs, cond_scale, rescaled_phi)
    ...
    def p_losses(self, x_start,condition,t, *, classes,noise = None):
