...
class MDM(nn.Module):
    ...
    def ddim_sample(self, classes, condition,shape, cond_scale = 1., rescaled_phi = 0, clip_denoised = True):#改
        ...
        #img = unnormalize_to_zero_to_one(img)
        img1 = unnorm_flow(img[:,:2,:,:])
        img2 = unnorm_flow(img[:,2:,:,:])
    ...
    def sample(self, classes, condition,cond_scale=6., rescaled_phi=0.7):  #改
        ...
        return sample_fn(classes, condition,(batch_size, (channels+1), h, w), cond_scale, rescaled_phi)#改
    ...
    def forward(self,flow,condition,*args, **kwargs):
        b, c, h, w, device, img_size, = *flow.shape, flow.device, self.image_size
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        return self.p_losses(flow,condition,t, *args, **kwargs)
